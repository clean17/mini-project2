...
public class ScrapController {
    ...
    public ResponseEntity<?> insertJobsScrap(@RequestBody UserInsertScrapReqDto sDto, @LoginUser LUser user) {
        if (sDto.getJobsId() == null) throw new CustomApiException("공고 번호가 필요합니다.");
        UserScrapOutDto result = scrapService.공고스크랩(1, sDto);
        return new ResponseEntity<>(new ResponseDto<>(1, "공고 스크랩 완료", result), HttpStatus.OK);
    }

    @DeleteMapping("/user/scrap/{id}/delete")
    public ResponseEntity<?> deleteJobsScrap(@PathVariable Integer id, @LoginUser LUser user) {
        if (id == null) throw new CustomApiException("스크랩 번호가 필요합니다.");
        scrapService.공고스크랩삭제(user.getId(), id);
        return new ResponseEntity<>(new ResponseDto<>(1, "공고 스크랩 삭제 완료", null), HttpStatus.OK);
    }

    @PostMapping("/comp/scrap/insert")
    public ResponseEntity<?> insertResumeScrap(@RequestBody CompInsertScrapReqDto sDto, @LoginComp LComp comp) {
        if (sDto.getResumeId() == null) throw new CustomApiException("이력서 번호가 필요합니다.");
        CompScrapOutDto result = scrapService.이력서스크랩(comp.getId(), sDto);
        return new ResponseEntity<>(new ResponseDto<>(1, "이력서 스크랩 완료", result), HttpStatus.OK);
    }

    @DeleteMapping("/comp/scrap/{id}/delete")
    public ResponseEntity<?> deleteResumeScrap(@PathVariable Integer id, @LoginComp LComp comp) {
        if (id == null) throw new CustomApiException("스크랩 번호가 필요합니다.");
        scrapService.이력서스크랩삭제(comp.getId(), id);
