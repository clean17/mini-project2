...
public class InterestController {
    ...
    public ResponseEntity<?> changeInterest(@RequestBody InterestChangeReqDto iDto, @LoginUser LUser user){
        if(ObjectUtils.isEmpty(iDto.getUserId())){
            throw new CustomApiException("유저아이디가 필요합니다.");
        }
        InterestChangeOutDto interDto = InterestService.관심수정(iDto, user.getId());
